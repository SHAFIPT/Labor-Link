...
export class LaborServices implements ILaborService {
  private laborRepsitory: ILaborSideRepository;

  constructor(laborRepsitory: ILaborSideRepository) {
  ...
  async fetchLabor(userLatandLog: {
    latitude: number;
    longitude: number;
  }): Promise<ILaborer[]> {
    try {
  ...
  async fetchSimilorLabors(
    ...
    return await this.laborRepsitory.fetchSimilorLabors(
      latitude,
      logitude,
      categorie,
      laborId
  ...
  async walletWithrow(
    ...
    return this.laborRepsitory.walletWithrow(laborId, amount, bankDetails);
  }

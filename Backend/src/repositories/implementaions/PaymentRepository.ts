...
export default class PaymnetRepository
  extends BaseRepository<IBooking>
  implements IPaymnetRepository
{
  constructor() {
    super(Booking);
  }
  ...
  async paymentSuccess(
    ...
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `http://localhost:5174/reviewRating?bookingId=${bookingId}`,
        cancel_url: "http://localhost:5174/userProfilePage",
        metadata: {
          bookingId,
          laborId,
          userId,
  ...
  async updateWebhook(event: Stripe.Event, sig: string): Promise<IBooking> {
    try {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        console.log("Received charge.updated event - no action needed");
        return null;
      }

...
const UserRegisterPage = () => {
  ...
  const handleSubmit = async (e : React.FormEvent) => {
    e.preventDefault();
     ...
     if (!criteria.length || !criteria.lowerCase || !criteria.upperCase || !criteria.unCommen) {
    // Show a validation error message to the user
    setError({ password: 'Password must meet all criteria: 8 characters, uppercase, lowercase, and not a common password.' });
    ...
    if (Otpresponce.status === 201 ) {
      dispatch(setModal(true))
    } else if (axios.isAxiosError(Otpresponce)) {
      toast.error(Otpresponce.message)
    }
  ...
  return (
    <div>
      ...
      <div className="LeftImage hidden lg:block lg:w-[60%] ml-16 mt-3 mb-7">
        <img src={LoginImage} alt="Login" className="w-full h-auto rounded-md  mb-7" />
      </div>
      ...
      <div className="flex flex-col lg:w-[50%] px-8">
          ...
          ></i>
           ...
           <div className="mt-2 text-md ">
            <ul className="list-disc ml-[20px] sm:ml-[20px] lg:ml-[94px] space-y-1 ">
              <li className={criteria.length ? 'text-green-500' : ''}>Must be at least 8 characters</li>
              <li className={criteria.lowerCase ? 'text-green-500' : ''}>Must include one lowercase character</li>
              <li className={criteria.upperCase ? 'text-green-500' : ''}>Must include one uppercase character</li>
              <li className={criteria.unCommen ? 'text-green-500' : ''}>Can't be too common</li>
            </ul>
          </div>
        ...
        >
          ...
          <div className="div ml-0 sm:ml-[0px] md:ml-[0px] lg:ml-[85px]">
            <Link to={'/login'} className="text-sm mt-[26px]">Already have an account ? <span><a href="#" className="hover:underline text-[#23c7b1]" >  Sign in</a></span></Link>

            <div className="text-sm mt-[26px]">Looking to join us as a Labor ?<span><a href="#" className="hover:underline text-[#23c7b1]">  Apply as a labor</a></span>
            </div>
          </div>
      </div>
    </div>  

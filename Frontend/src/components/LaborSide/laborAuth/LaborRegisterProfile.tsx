 ...
 const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) : void => {
  ...
  const handleFormsubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      formDataForAPI.append('skill', skill);
          ...
          <div className="flex space-x-5 items-center">
            ...
            //  onClick={() => document.getElementById("fileInput")?.click()}
             >
              ...
              <img
               src={
            typeof image === "string" && image? image : image instanceof File ? URL.createObjectURL(image): "/default-profile-image.png" }
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                {daysOfWeek.map((day) => (
                  <div className="flex items-center" key={day.toLowerCase()}>
                    <input type="checkbox" id={day.toLowerCase()} className="mr-2"
                    checked={availability.includes(day.toLowerCase())}
                      onChange={handleCheckboxChange} value={availability} />
          ...
          <div className="rightDive space-y-7">
           <div className="relative flex flex-col">
              ...
              <input
               ...
               value={skill}
               onChange={handleInputChange(setSkill)}
               />
               {showErrors && error?.skill && (
                    <p className="text-red-500 text-sm mt-1">{error.skill}</p>
                  )}
            </div>
            <div className="flex flex-col">
